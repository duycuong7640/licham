...
let vietnameseDateInputCounter = 0;
...
function setupVietnameseDateInput(input, options = {}) {
    if (!input) return null;

    const dataMinYear = Number(input.dataset.minYear);
    const dataMaxYear = Number(input.dataset.maxYear);
    const minYear = Number.isInteger(options.minYear)
        ? options.minYear
        : Number.isInteger(dataMinYear)
            ? dataMinYear
            : 1900;
    const maxYearCandidate = Number.isInteger(options.maxYear)
        ? options.maxYear
        : dataMaxYear;
    const maxYear =
        Number.isInteger(maxYearCandidate) && maxYearCandidate >= minYear
            ? maxYearCandidate
            : Math.max(minYear, 2050);
    const preferredYear =
        Number(input.value.slice(-4)) || new Date().getFullYear();
    const defaultYear = Math.min(maxYear, Math.max(minYear, preferredYear));

    const parse = (value = input.value) => {
    ...
    if (
        Number(input.value.slice(-4)) < minYear ||
        Number(input.value.slice(-4)) > maxYear
    ) {
        input.value = `01/01/${String(defaultYear).padStart(4, "0")}`;
    }

    const field = input.parentElement;
    const picker = document.createElement("div");
    vietnameseDateInputCounter += 1;
    const pickerId = input.id
        ? `${input.id}-select-picker`
        : `date-select-picker-${vietnameseDateInputCounter}`;
    picker.id = pickerId;
    picker.className = "date-select-picker";
    picker.hidden = true;
    picker.innerHTML = `
    <select class="date-select-day" aria-label="Chọn ngày"></select>
    <select class="date-select-month" aria-label="Chọn tháng"></select>
    <select class="date-select-year" aria-label="Chọn năm"></select>
  `;
    field?.classList.add("date-picker-field");
    field?.append(picker);

    const daySelect = picker.querySelector(".date-select-day");
    const monthSelect = picker.querySelector(".date-select-month");
    const yearSelect = picker.querySelector(".date-select-year");
    const pad = (number) => String(number).padStart(2, "0");

    monthSelect.innerHTML = Array.from(
        { length: 12 },
        (_, index) => `<option value="${index + 1}">${pad(index + 1)}</option>`,
    ).join("");
    ...
    const fillDays = (year, month, selectedDay = 1) => {
    ...
    const syncPicker = () => {
        const date = parse() || new Date(defaultYear, 0, 1, 12);
        yearSelect.value = String(date.getFullYear());
        monthSelect.value = String(date.getMonth() + 1);
        fillDays(date.getFullYear(), date.getMonth() + 1, date.getDate());
    };
    ...
    const updateInput = () => {
        ...
        fillDays(year, month, selectedDay);
        input.value = `${pad(daySelect.value)}/${pad(month)}/${year}`;
    ...
    const showError = () => {
        input.setCustomValidity(
            `Nhập ngày hợp lệ theo định dạng dd/mm/YYYY, trong khoảng năm ${minYear}–${maxYear}.`,
        );
...
function init() {
    ...
    const vietnameseDateFields = new Map();
    $$(".vietnamese-date-input").forEach((input) => {
        vietnameseDateFields.set(input, setupVietnameseDateInput(input));
    });

    const convertType = $("#convertType");
    const convertDateInput = $("#convertDate");
    const lunarDaySelect = $("#lunarInputDay");
    const lunarMonthSelect = $("#lunarInputMonth");
    const lunarYearSelect = $("#lunarInputYear");
    const convertDateField = vietnameseDateFields.get(convertDateInput) ?? null;
    const convertMinYear = convertDateField?.minYear ?? 1900;
    const convertMaxYear = convertDateField?.maxYear ?? 2050;
    const defaultConvertYear = convertDateField?.defaultYear ?? 2026;
    if (lunarDaySelect) {
    ...
    if (lunarYearSelect) {
        lunarYearSelect.innerHTML = Array.from(
            { length: convertMaxYear - convertMinYear + 1 },
            (_, index) => {
                const year = convertMinYear + index;
                return `<option value="${year}">${year}</option>`;
            },
        ).join("");
        lunarYearSelect.value = String(defaultConvertYear);
    }
    ...
    $("#convertForm")?.addEventListener("submit", (event) => {
        ...
        const date = isLunarInput
            ...
            : convertDateField?.parse();
        if (!date) {
            convertDateField?.showError();
            return;
        }
