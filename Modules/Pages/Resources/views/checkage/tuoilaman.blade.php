...
@section('content')
    ...
    <div class="day-detail-page">
        <div class="day-page-heading">
            <p class="eyebrow">Xem hai tuổi hợp khắc theo năm sinh</p>
        ...
        @if(!empty($row['reading']))
            <div class="wrap-content-love pd-0">
                ...
                <div class="love-result-box pd-0 mb-0">
                    <h2>Danh sách tuổi hợp làm ăn với {{ $row['owner']['canChi'] }} {{ $row['owner']['lunarYear'] }}</h2>
            ...
            <div class="check-age-card check-age-result">
                <div class="check-age-result-head">
                    <div>
                        ...
                        <h2>
                            Tuổi {{ $row['owner']['canChi'] }} {{ $row['owner']['lunarYear'] }} hợp làm ăn với tuổi nào?
                        </h2>
                        <p>
                            Xem danh sách các tuổi hợp với
                            tuổi {{ $row['owner']['canChi'] }} {{ $row['owner']['lunarYear'] }} theo Can Chi,
                ...
                <div class="check-age-ranking mb-6">
                    ...
                    @if(!empty($row['summary']['bestBusinessAges']))
                        @php
                            $arr = array_merge($row['summary']['bestBusinessAges'], $row['summary']['goodBusinessAges'], $row['summary']['canCooperateSmall'])
                        @endphp
                                ...
                                <div class="check-age-status-wrap" tabindex="0">
                                    ...
                                    <div class="check-age-tooltip">
                                        <strong>Vì sao
                                            ...
                                            phù hợp?</strong>
                                        <p>{{ $value['businessAdvice'] }}</p>
                                        @if(!empty($value['risks']))
                                            <p><strong>Rủi do:</strong> {{ implode(', ', $value['risks']) }}</p>
                                        @endif
                ...
                <div class="check-age-result-head">
                    <div>
                        ...
                        <h2>
                            Tuổi {{ $row['owner']['canChi'] }} {{ $row['owner']['lunarYear'] }} không hợp làm ăn với tuổi nào?
                        </h2>
                        <p>
                            Danh sách các tuổi không hợp
                            với {{ $row['owner']['canChi'] }} {{ $row['owner']['lunarYear'] }} được tham khảo theo
                ...
                <div class="check-age-ranking">
                    ...
                    @if(!empty($row['summary']['notRecommendedAges']))
                        @php
                            $arr = array_merge($row['summary']['notRecommendedAges'], $row['summary']['avoidAges'])
                        @endphp
                        @foreach($arr as $k=>$value)
                            <div class="check-age-ranking-row @if($k <= 2) check-age-top @endif">
                                <span class="check-age-rank">{{ $k + 1 }}</span>
            ...
            <section class="check-age-wrap">
                <div class="check-age-content">
                    ...
                    <p>
                        Việc xem tuổi hợp – khắc giúp tham khảo mức độ tương hợp giữa hai tuổi theo quan niệm dân gian,
                        dựa trên Can Chi, Ngũ hành, Nạp âm và cung mệnh. Danh sách tuổi hợp hoặc không hợp chỉ là một
                        góc
                        nhìn tham khảo; trong thực tế, một mối quan hệ bền vững vẫn phụ thuộc nhiều vào sự thấu hiểu,
                        tôn trọng, sẻ chia và cách ứng xử của mỗi người.
                    </p>
                </div>
            </section>
        @endif
